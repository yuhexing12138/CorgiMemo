...
- **面板行距节奏（v2026-09-21 用户要求"H 面板行距参照 A 面板"）**：A 面板节奏 = 每行/分区上下各 **6dp**（`BlockColorRow` 的 vertical padding，横向 12dp）→「标签→内容」6dp、「相邻行/分区之间」12dp，内容区上下另加 4dp。H 面板已逐项对齐：`SectionTitleVerticalPadding = 6.dp` / `HeadingPanelVerticalPadding = 4.dp`，因此**分区之间不放额外 Spacer**（放了就比 A 面板松）。⚠️ 改 H 面板间距时按此节奏，别再引入 Spacer。
