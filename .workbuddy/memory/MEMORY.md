...
- ★★ 自链接「」显式包裹契约（v2026-09-24 终版，用户决策——**显式标记取代 URL 形态正则**）：导出 `bracketSelfLinks` 把显示文本==href 的 link 塌成 `「href」` text（blocksToMd ①a，颜色编码前）；载入 `splitBracketLinks` 匹配 `「(https?://[^」]*)」` 还原 link（**仅认协议前缀防误伤普通中文引用**；精确边界无前置判定无尾标点剥离）；autolinkBareUrls 两级拆分：「」优先 → 剩余文本走裸 URL autolink（存量/手打兼容）。官方 formatLink「显示==URL 退化为裸 URL」随之失效。回归 `tests/markdown/link-contract.test.ts`（21 例）。
- ★★ 自链接（显示文本==href）文字编辑 → href 同步跟随：自定义扩展 `linkHrefSync.ts`（createExtension **对象重载**挂 `appendTransaction` PM 插件，`transform.mapping` 映射旧 link 区间、新文档收集同 href 相邻片段、removeMark+addMark(href=新全文)）。取舍：显示≠href 的链接改文字 href 不变；删空=解除链接；多 text 节点自链接不触发（安全降级）；PM 一轮机制无循环。
