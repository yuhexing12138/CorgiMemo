...
- **块级撤销栈（第二轮补充）**：controller 内 `ArrayDeque<String>` markdown 快照栈（50 条），结构操作（插图/拆块/合并/删块/重排）前 `pushBlockSnapshot()`；`undoBlocks()/redoBlocks()` 快照重建 + 焦点回第一块；屏幕按钮 = 块级栈 ∨ 聚焦块内 history，**块级优先**（输入走块内 history，结构变更走快照，互斥）。
