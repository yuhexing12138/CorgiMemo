...
- **块内水平对齐常量（2026-09-09）**：Text 块文字左右缘 = 块内容边界 ± 16dp（RichTextEditor `contentPadding(start/end=16.dp)`）；非文本块（图片等）要"与文本左右对齐"必须用同一数值——`BodyBlocksEditor` 里的 `BLOCK_CONTENT_PADDING = 16.dp`，改动需双向同步。
- **块级图片撑满宽度（2026-09-09）**：`InlineImagePreview(fillMaxWidth = true)` → 宽度 `fillMaxWidth()`、高度 `aspectRatio(painter 真实比例)`（严格等比不限高），撑满态内部**不加**水平 padding（由调用方给 16dp）；旧路径（maxWidth=300.dp）保持不变。
- **块重排必须加 key（2026-09-09）**：`sh.calvin.reorderable.ReorderableColumn`（非 Lazy 版）内部是 `Column { list.forEachIndexed }`，**item 无 key** → 交换后槽位被复用渲染另一个块 → 图片块 `SubcomposeAsyncImage` 重载、高度塌陷到占位高再弹回（"交换瞬间上下跳动"）。`BlocksReorderableColumn` 已加 `itemKey` 参数并在 `ReorderableItem` 内 `key(itemKey(item))`，调用方必须传**块 id**。同类隐患：库 `remember(list, spacing)` 按结构相等比较，若块改成 data class 或两块内容相同 → state 不重建 → 拖拽 offset 残留错位。
- **图片宽高比缓存（2026-09-09）**：`InlineImagePreview` 内进程级 `ImageAspectRatioCache`（path→宽/高），success 写入、loading/error 读取；撑满态占位高度用 `aspectRatio(cached)` 而非固定 180dp，避免任何重载场景的高度塌陷。
- **图片选中工具栏必须 Popup 独立窗口（2026-09-09）**：布局内渲染（AnimatedVisibility 画在块 Box 里）会被**后续兄弟块**盖住/拦截触摸——父级 Column 后组合者画上层，工具栏 clamp 到 viewport 边界时正好延伸进下一块图片区域。已迁 `Popup(alignment=TopStart, offset=组合期IntOffset, PopupProperties(focusable=false, clippingEnabled=false))`，锚点 = 内层 Column（frame 实测坐标同基准零补偿）。**Popup 动画两陷阱**：① 首组合时 visible 已 true 会丢 enter → 内容内加 `toolbarAnimatedIn` 先 false 一帧再置 true；② visible=false 直接卸载 Popup 会截断 exit → 挂载状态按 `ImageToolbarPopupUnmountDelayMillis` 延迟卸载（≥ 退场时长+余量），快速重选 delay 被取消动画反向接续。工具栏 shadow 不被裁（PopupLayout 含 FLAG_LAYOUT_NO_LIMITS）；bounds 外触摸穿透底层（WATCH_OUTSIDE_TOUCH）。
- **图片缩小/恢复动画与工具栏退场同步（2026-09-09）**：`animateFloatAsState` 驱动 `widthFraction`（1f⇄0.5f）——图片**真实缩放**（高度 aspectRatio 逐帧跟随、备注宽度同步跟随），**不用 animateContentSize**（内部 clipToBounds 容器裁切式，内容瞬间变小）；与工具栏退场共用 `ImageScaleAnimationDurationMillis=250`——toggleImageShrunk 同帧翻转 shrunk+清选中 → 同帧启动等长同结；工具栏位置用冻结快照原地退场不受动画影响；Popup 卸载延迟 `ImageToolbarPopupUnmountDelayMillis=350` ≥ 动画时长+余量；undo 翻转 shrunk 动画自动跟随。
- **图片间必须留空行（2026-09-09）**：任意两个 Image 块之间必须有空 Text 块。`InsertImageSeparatorCommand`（纯插入、不动焦点）+ `buildImageSeparatorCommands()`（自后向前扫相邻对）+ `insertBlockAt/removeBlockById` 原语；单张插图走 `executeAndPushWithImageSeparators`，批量插图整批后统一补，拖拽 `moveBlock` 把 move+补块打包 CompositeCommand（一步撤销，revert 逆序）。空块序列化走既有 `EMPTY_BLOCK_PLACEHOLDER` 往返。删除/加载路径**不做**兜底（用户决策）。
- **图片间空行阅读态不渲染（2026-09-09）**：详情页 `InspirationBodyRichText` 用 `skipRenderIndexes`（空白段且前后最近非空白段都是图片段 → 跳过）；首页走源头 `plainText()`（空 Text 块前后最近非空白块都是 Image → 不输出行）。**Kotlin isBlank() 不认 NBSP**（isWhitespace('\u00A0')=false），判空须先剔除 NBSP/ZWSP。旧数据不迁移（用户决策），仅新保存生效。
