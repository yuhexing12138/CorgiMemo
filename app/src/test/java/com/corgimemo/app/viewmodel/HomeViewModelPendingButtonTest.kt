...
/**
 * HomeViewModel "待完成"区域展开/折叠相关 ViewModel 逻辑单元测试
 *
 * 覆盖：
 * - showPending 默认值为 true
 * - toggleShowPending 翻转状态并触发持久化
 ...
 * （showCompleted/showPinned/showPending/hideDetails/hideCompletedItems），
...
class HomeViewModelPendingButtonTest {
    ...
    fun setUp() = runTest {
        ...
        coEvery { mockCorgiPreferences.showPending } returns flowOf(true)
     ...
     * 场景：用户点击待完成按钮
     * 预期：
     * - showPending 状态翻转（true → false 或 false → true）
