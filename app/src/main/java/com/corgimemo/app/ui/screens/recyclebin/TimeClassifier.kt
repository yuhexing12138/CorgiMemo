 ...
 * 提供两个核心方法：
 * - [classifyByTime]: 把时间戳归类到 [DeletedGroupKind]
...
object TimeClassifier {
     ...
     * @return 分组枚举
     */
    fun classifyByTime(deletedAt: Long, now: Long): DeletedGroupKind {
        val days = daysBetween(deletedAt, now)
        return when {
            days == 0L -> DeletedGroupKind.TODAY
            days == 1L -> DeletedGroupKind.YESTERDAY
            days in 2L..6L -> DeletedGroupKind.THIS_WEEK
            else -> DeletedGroupKind.EARLIER
        }
    }
