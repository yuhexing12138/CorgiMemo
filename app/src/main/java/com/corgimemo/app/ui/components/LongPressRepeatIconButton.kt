 ...
 *    （`SuspendingPointerInputFilter.kt` 第 121-128 行的设计说明：刻意如此，避免破坏结构化并发），
 *    直接 `launch` 不可用。因此本组件在组合层用 `rememberCoroutineScope()` 拿到
 *    Composable 的作用域来承载连发协程——**手势检测（受限域）与连发定时器（普通域）彻底分离**：
 *    受限域只负责「何时开始/停止」，普通域负责「按节拍重复」。
 *
 * 3. **`IconButton` 的 `clickable` 会在 down 时立即 `consume()` 事件**
 *    （参见本项目 `PressFeedback.kt` 第 180-196 行的同款踩坑记录）。
 *    故本组件**不用 onClick 承载动作**（传空实现），动作完全由
 *    `awaitFirstDown(requireUnconsumed = false)` + `waitForUpOrCancellation()` 驱动，
 *    避免「点击」与「连发」各触发一次造成双执行。
 *
...
fun LongPressRepeatIconButton(
    ...
    IconButton(
        /** 动作不走 onClick（见「约束 3」），完全交给下方 pointerInput */
        onClick = {},
        enabled = enabled,
        modifier = modifier
            .size(buttonSize)
            .pointerInput(enabled, initialDelayMs, repeatIntervalMs) {
