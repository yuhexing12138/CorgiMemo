 ...
 * 「我的」页面主屏
 *
 * v1.4 简化：仅保留 3 模块单列 LazyColumn
 * ① 柯基展示头卡（ProfileHeroCard，用户信息卡）
 * ② 主题配色卡（ThemeQuickSwitch，整卡点击跳 Appearance）
 * ③ 成就统计卡（AchievementSummaryCard，点击"查看全部"跳 Achievement）
 *
 * v1.4 已外移内容（移至柯基互动页 CorgiDetailScreen）：
 * - 装扮入口卡（→ CorgiDetailScreen，含"切换装扮"按钮 + OutfitBottomSheet）
 * - 7 天情绪图表（→ CorgiDetailScreen，情绪值区可展开）
 * - 累计完成 / 连续天数 / 情绪值三栏统计（→ CorgiDetailScreen 柯基卡牌区）
 *
 * v1.1 调整说明：
