...
object MarkdownParser {
     ...
     * - 普通链接 `[文字](url)` → 保留「文字」
     * - 引用符 `>`、ATX 标题 `#`、分割线 → 移除
     *   **分割线两种形态都要剥**（v2026-09-23 补）：裸 `---` / `***` / `___`，
     *   以及本项目带样式载体 `--- dashed` / `--- wavy`
     *   （`--- <style>` 符号后有后缀，裸分割线正则匹配不到，曾漏进摘要）
    ...
    fun toPlainText(markdown: String): String {
        ...
        // 6a) **带样式**的分割线载体 `--- dashed` / `--- wavy`（v2026-09-23 新增）
        //     ——本项目自编码的样式载体（见 BodyBlocksController.DIVIDER_MD_DASHED / _WAVY，
        //     WebView 侧 blocksToMd 产出 `--- <style>`）。该行**独占一段**，
        //     故整行连同行尾换行一起删、不留空格占位。
        //     ⚠️ **必须先于 6b) 执行**：6b) 的裸分割线正则同样能匹配行首的 `---`，
        //     若先跑 6b) 会把前缀吃掉、把后缀 `dashed` 单独留在摘要里。
        //     （此前该规则在 6b) 位置、靠 `\s*$` 行尾锚与先后次序侥幸正确；
        //     本次把「带样式」提到前面，是让正确性由**顺序**保证、而非依赖锚点。）
        text = text.replace(Regex("""(?m)^[ \t]*[-*_]{3,}[ \t]+(?:dashed|wavy)[ \t]*$\n?"""), "")
        // 6b) **裸**分割线 --- / *** / ___（stripMarkdown 不处理，单独去除）
        //    正则要求**整行仅由同一符号构成**（`([ \t]*\1){2,}` 即同符号重复 ≥3 次）。
        //    ⚠️ 两处刻意的写法（v2026-09-23 修）：
        //    ① 行尾锚 `[ \t]*$` **必须保留**——去掉它会让带样式的载体前缀被单独吃掉；
        //    ② 替换串是**空串 + 吃掉行尾换行 `\n?`**，而非原来的空格占位——
        //       分割线是**独占一段**的块级元素，用空格占位会把该行变成"只含一个空格
        //       的非空行"，它会**躲过空行折叠**（`\n \n` 不匹配 `\n{2,}`）⇒ 摘要里
        //       上下两行之间凭空多出一整行（真机：「测试分割线」卡片）。
        //       该行本就没有任何文字，不需要空格来"隔开相邻词"。
        text = text.replace(Regex("""(?m)^[ \t]*([-*_])([ \t]*\1){2,}[ \t]*$\n?"""), "")
