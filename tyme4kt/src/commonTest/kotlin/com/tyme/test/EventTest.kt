...
class EventTest {
    ...
    fun test0() {
        ...
        assertNotNull(d)
        assertEquals("2008年2月29日", d.toString())

        // 2005年没有2月29，按最初设置的，没有就倒推1天
        d = e.getSolarDay(2005)
        assertNotNull(d)
        assertEquals("2005年2月28日", d.toString())

        e = Event.fromName("农历生日")
        assertNotNull(e)
        d = e.getSolarDay(2026)
        assertNotNull(d)
        assertEquals("2026年6月6日", d.toString())
    }

    @Test
    fun test1() {
        init()
        val e = Event.fromName("国际消费者权益日")
        assertNotNull(e)
        assertEquals("国际消费者权益日", e.getName())

        val d = e.getSolarDay(2026)
        assertNotNull(d)
        assertEquals("2026年3月15日", d.toString())
    }
}
